 ...
 */
OdbcWrapper::OdbcWrapper(std::string dsn, std::string user, std::string pw) :
 ...
 * Connect to the DSN.
 */
odbc_rc OdbcWrapper::connect() {
  long rc;
  odbc_rc err = ODBC_SUCCESS;
...
odbc_rc OdbcWrapper::disconnect() {
 ...
 */
odbc_rc OdbcWrapper::submit_query(std::string query) {
  long rc;
 ...
 */
odbc_rc OdbcWrapper::get_row(char *row_buffer) {
  long rc;
  SQLSMALLINT columns;
  SQLUSMALLINT i;
  char buf[512];
